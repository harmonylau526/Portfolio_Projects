...
colnames(data.matrix) <- c(
  paste("wt", 1:5, sep=""),    #fake sample
  paste("ko", 1:5, sep=""))
rownames(data.matrix) <- paste("gene", 1:100, sep="")    #fake variables
for (i in 1:100) {
  wt.values <- rpois(5, lambda=sample(x=10:1000, size=1))     #fake reading, poisson distributions
  ko.values <- rpois(5, lambda=sample(x=10:1000, size=1))
 
  data.matrix[i,] <- c(wt.values, ko.values)
}
head(data.matrix)      #variables are rows, observations/amples are columns
dim(data.matrix)
 #prcomp expects variables to be columns and samples to be rows, so Josh used t() to transpose the matrix
cleaned <- SotTCombined2010 %>%
  drop_na() %>%
  remove_rownames %>% column_to_rownames(var="Country")

cov.mat
pca <- prcomp(cleaned, scale=TRUE) 
summary(pca)
pca$x
X=pca$x[,1]
Sample=colnames(pca$x)
## plot pc1 and pc2
...
svd.stuff <- svd(scale(cleaned, center=TRUE))
 
## calculate the PCs
svd.data <- data.frame(Sample=rownames(cleaned),
...
## original data
svd.pcs <- t(t(svd.stuff$v) %*% t(scale(cleaned, center=TRUE)))
svd.pcs[,1:2] ## the first to principal components
 
svd.df <- ncol(cleaned - 1)
...
##
############################################
cov.mat <- cov(scale(cleaned, center=TRUE))
dim(cov.mat)
...
eigen.pcs <- t(t(eigen.stuff$vectors) %*% t(scale(cleaned, center=TRUE)))
eigen.pcs[,1:2]
